...
export default function App() {
  ...
  const calculateTotalPenalties = (activeConditions: ActiveCondition[]) => {
    ...
    activeConds.forEach(cond => {
      ...
      // States - Always include the condition name to ensure it shows up
      states.add(cond.name);
      ...
      commonStates.forEach(s => {
        if (summary.includes(s)) states.add(s);
      });
    });
                                      ...
                                      <div className="flex flex-col gap-4">
                                        ...
                                        <div>
                                          ...
                                          <div className="space-y-4">
                                            ...
                                            <div className="flex flex-wrap gap-x-4 gap-y-2 pb-3 border-b border-zinc-800/50">
                                            ...
                                            {/* Full Descriptions of Active Conditions */}
                                            <div className="space-y-3">
                                              {currentCombatant.activeConditions.map(ac => {
                                                const cond = conditions.find(c => c.id === ac.conditionId);
                                                if (!cond) return null;
                                                return (
                                                  <div key={ac.conditionId} className="flex flex-col gap-1 border-l-2 border-primary/40 pl-3 py-1 bg-zinc-950/30 rounded-r-lg">
                                                    <div className="text-[10px] font-black text-primary uppercase tracking-wider flex items-center justify-between">
                                                      <span>{cond.name}</span>
                                                      {ac.remainingTurns !== null && (
                                                        <span className="text-[8px] text-zinc-500 lowercase font-normal bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-800">
                                                          {ac.remainingTurns} turnos
                                                        </span>
                                                      )}
                                                    </div>
